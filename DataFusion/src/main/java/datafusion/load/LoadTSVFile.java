...
public class LoadTSVFile {
						...
						for (int i = 2; i < fields.length; i++) {
							...
							if (fields[i].length() > 0)
								if (j < orderedAttributeNames.length) {
									Attribute attrToAdd = getAttributeFromString(orderedAttributeNames[j], fields[i],
											attributeDataTypes[j], attributeTypes[j]);
									if (attrToAdd != null)
										rec.addAttribute(attrToAdd);
								} else {
									Attribute attrToAdd = getAttributeFromString(String.format("Attr%04d", j),
											fields[i], attributeDataTypes[j], attributeTypes[j]);
									if (attrToAdd != null)
										rec.addAttribute(attrToAdd);
								}
						}
	...
	public RecordCollection loadGold(File file) {
		String line = "";
		try {
			ArrayList<Record> records = new ArrayList<Record>(1000);
			BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(file), "UTF-8"));
			line = reader.readLine();
			int lineNo = 0;
			while (line != null) {
	...
	protected Attribute getAttributeFromString(String name, String rawValue, AttributeDataType dataType,
			AttributeType type) {
		switch (dataType) {
		case FLOAT: {
