...
/**
 * A data structure corresponding to a row in a relational database. Each record
 * has an associated entity and the entity has a number of attribute associated
 * with it. Each row also has a source associated with the row. This source
 * could be the website providing the data or an algorithm if the row appears in
...
public class Record {
	...
	/**
	 * Constructor for record. Inits record w/ random id number and no attributes*
	 * 
	 * @param source
	 * @param entity
	 */
	...
	/**
	 * Constructor which initializes all fields by id number and gives the record a
	 * random id
	 * 
	 * @param source
	 * @param entity
	...
	/**
	 * Creates a new record with all fields specified, each attribute is added to
	 * the hash map
	 * 
	 * @param id
	 * @param source
	...
	/**
	 * TODO: Is this definition correct? Two records are equal if they have the same
	 * source and entity?
	 * 
	 * @param obj
	 * @return
	...
	public boolean equals(Object obj) {
		return (obj instanceof Record) && this.id == ((Record) obj).getId()
				&& this.source.equals(((Record) obj).getSource()) && this.entity.equals(((Record) obj).getEntity());
	}
	...
	/**
	 * Adds the attribute to the record. If the record already has an attribute with
	 * the same name, the old value will be overwritten
	 * 
	...
	/**
	 * Returns the attribute the record has with the given name or null if the
	 * record doesn't store the attribute with this name
	 * 
	 * @param attrName
	 * @return
	 */
